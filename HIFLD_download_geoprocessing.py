...
Some ancillary fields interfere with geopandas ability to open the CSV and need to be deleted.
""" 
...
def getData(dataCatalog, outputDir, startIndex, endIndex):
    ...
    for i in range(len(dataCatalog)):
        if i > startIndex and i < endIndex:
            ...
            if (status_code == 200) and (list(geoJson.keys())[0] != 'error'):
...
def bufferIntersect(directory, buffer_size):
    ...
    gdf = GeoSeriesToGeoDataFrame(intersect, 'MultiPolygon')
...
def GeoSeriesToGeoDataFrame(geoseries, geom_type):
    gdf = gpd.GeoDataFrame()
    gdf['geometry'] = geoseries[geoseries.geom_type == geom_type]
...
outputDir = 'HIFLD_requests2'

# Run Functions
getData(dataCatalog, outputDir, 23, 30)
bufferIntersect('C:/test/HIFLD_requests2/', 0.6)
